...
def _local_content(*, business_type: str, business_name: str, product_name: str,
    ...
    return {"headline": headline, "subheadline": subheadline, "caption": caption, "hashtags": tags,
            "cta": "DM us to order" if not phone.strip() else f"Call {phone.strip()} today", "design": _brand_design({**kwargs, "business_name": business_name, "business_type": business_type, "product_name": product_name, "price": price, "location": location, "phone": phone, "has_photo": has_photo})}


def _local_v2(*, business_type: str, business_name: str, product_name: str,
              offer: str, price: str, location: str, phone: str,
              additional_info: str, **_: Any) -> dict[str, Any]:
    brand = business_name.strip() or business_type.strip().title(); product = product_name.strip(); hook = offer.strip() or f"You need to try {product}"
    return {
        "reel_ideas": [
            {"title": "Problem → Solution", "hook": f"Still looking for {product}?", "script": ["Show the problem in the first 2 seconds.", f"Reveal {product} with a quick close-up.", f"Show the result/benefit and finish with: {hook}."], "duration": "10–15 sec"},
            {"title": "3 Reasons to Try It", "hook": f"3 reasons {product} is worth trying 👀", "script": [f"Reason 1: Show {product}.", f"Reason 2: Highlight {offer or 'the key benefit'}.", f"Reason 3: Show the final result and invite viewers to contact {brand}."], "duration": "15–20 sec"},
            {"title": "Behind the Scenes", "hook": f"How we make {product} ✨", "script": ["Show a fast behind-the-scenes clip.", "Cut through 3–4 satisfying process shots.", f"End on the finished {product} with a clear CTA."], "duration": "12–18 sec"},
        ],
        ...
        "posting_suggestion": {"best_days": ["Tuesday", "Thursday", "Saturday"], "best_time": "6:00 PM–8:00 PM local time", "frequency": "3–4 posts/reels per week", "tip": "Test these slots for 2 weeks, then keep the times that produce the strongest saves, shares, and comments."},
    }


async def generate_content(*, include_v2: bool = False, **kwargs: Any) -> dict[str, Any]:
    api_key = os.getenv("OPENAI_API_KEY", "").strip(); base = _local_content(**kwargs); v2 = _local_v2(**kwargs) if include_v2 else {}
    if not api_key: return {**base, **v2}
    from openai import AsyncOpenAI
    client = AsyncOpenAI(api_key=api_key); model = os.getenv("OPENAI_MODEL", "gpt-4o-mini")
...
hashtags: array of 5 strings. reel_ideas: array of 3 objects with title, hook, script (array), duration.
...
Brand colors: primary {brand_primary_color}, secondary {brand_secondary_color}, accent {brand_accent_color}
Brand font: {brand_font_family}
Social handle: {brand_social_handle}
""".format(**kwargs)
    try:
        response = await client.chat.completions.create(model=model, temperature=0.8, response_format={"type": "json_object"}, messages=[
            {"role": "system", "content": "You are an expert local-business social media strategist. Respect the provided brand identity in tone and copy."},
            {"role": "user", "content": task},
        ])
        data = json.loads(response.choices[0].message.content or "{}"); merged = {**base, **v2, **data}
